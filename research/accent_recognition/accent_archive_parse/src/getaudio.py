...
class GetAudio:
    ...
    def __init__(self, csv_filepath, destination_folder= 'audio/', wait= 1.5, debug=False ):
        ...
        self.audio_df = pd.read_csv(csv_filepath, sep='\t')
        self.url = 'https://accent.gmu.edu/soundtracks/{}.mp3'
    ...
    def get_audio(self):
        ...
        for lang_num, category in zip(self.audio_df['language_num'], self.audio_df['category']):
            if not os.path.exists(self.destination_folder +'{}.wav'.format(lang_num)):
                ...
                #print(self.url.format(lang_num))
                #print(filename)
                sound = AudioSegment.from_mp3(filename)
                if not os.path.exists('../' + self.destination_folder + category):
                    os.makedirs('../' + self.destination_folder + category)
                sound.export('../' + self.destination_folder + category + '/' + "{}.wav".format(lang_num), format="wav")
                counter += 1
